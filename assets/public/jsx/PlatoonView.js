...
ReactView.init = function(actions, platoonsStore, elem) {
	...
	var PlatoonUserRow = React.createClass({
		displayName: 'PlatoonUserRow',
		render: function() {
	...
	var PlatoonsView = React.createClass({
		displayName: 'PlatoonsView',
		...
		render: function() {
			var users = _.map(this.state.currentPlatoon.users, function(user) {
				return React.createElement(PlatoonUserRow, {
					key: user.name,
					user: user
				});
			});
			...
			return (
				React.createElement("div", {
					...
					React.createElement("select", {
							id: "select-platoon",
							onChange: this.handleChangePlatoon,
							ref: "platoon"
						},
	...
	React.render(
		React.createElement(PlatoonsView, {
			platoons: platoonsStore.getDefaultProps()
		}),
		elem
	);
}