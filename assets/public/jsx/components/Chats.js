	...
	render: function() {
		...
		var chats = _.map(this.state.chats, function(chat) {
			...
			return React.createElement(ChatRow, {
				key: chat.chatId,
				chat: chat,
				isSelected: chat.chatId == ChatStore.getCurrentChat()
			});
		});

		if (!chats.length) {
			chats = React.createElement("li", {
				className: "no-chats"
			}, locale.t("ID_MOBILE_NO_CHATS"));
		}
		...
		return (
			React.createElement("div", {
				...
				React.createElement("div", {
					...
					}, locale.t('ID_MOBILE_CHAT_CREATE_NEW'))
				),
				React.createElement("div", {
